 ...
 * 计算增益选项带来的增益。获取增益集合
 */
export const 根据增益选项获取增益集合 = (增益数据: 增益选项数据类型): 属性加成[] => {
  let 增益集合: 属性加成[] = []
...
export const 通用增益计算 = (增益: 属性加成, 最终计算属性: 最终计算属性类型): 最终计算属性类型 => {
  ...
  switch (属性) {
    ...
    case 属性类型.全能等级:
      ...
      break
    // 基础攻击
    case (属性类型.全基础攻击, 属性类型.外功基础攻击, 属性类型.内功基础攻击):
      计算后人物属性.基础攻击 += 值
      break
    case (属性类型.郭氏全基础攻击, 属性类型.郭氏外功基础攻击, 属性类型.郭氏内功基础攻击):
      郭氏基础攻击 = 郭氏基础攻击 + 值
      break
    // 破防
    case (属性类型.全破防等级, 属性类型.外功破防等级, 属性类型.内功破防等级):
      计算后人物属性.破防等级 += 值
      break
    case (属性类型.郭氏全破防等级, 属性类型.郭氏外功破防等级, 属性类型.郭氏内功破防等级):
      郭氏破防等级 = 郭氏破防等级 + 值
      break
    // 会心
    case (属性类型.全会心等级, 属性类型.外功会心等级, 属性类型.内功会心等级):
      计算后人物属性.会心等级 += 值
      break
    case (属性类型.全会心等级百分比, 属性类型.外功会心百分比, 属性类型.内功会心百分比):
      额外会心率 = 额外会心率 + 值
      break
    // 会心效果
    case (属性类型.全会心效果等级, 属性类型.外功会心效果等级, 属性类型.内功会心效果等级):
      计算后人物属性.会心效果等级 += 值
      break
    case (属性类型.郭氏全会心效果等级,
    属性类型.郭氏外功会心效果等级,
    属性类型.郭氏内功会心效果等级):
      ...
      break
    // 无视防御相关
    case (属性类型.内功基础防御, 属性类型.外功基础防御):
      当前目标 = {
        ...当前目标,
        防御点数: Math.max(当前目标.防御点数 + 值, 0),
      }
      break
    case (属性类型.郭氏外功基础防御, 属性类型.郭氏内功基础防御, 属性类型.郭氏全无视防御):
      郭式无视防御 = 郭式无视防御 + 值
      break
    // 加速等级
    case 属性类型.加速等级:
