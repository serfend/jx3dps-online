...
import { 完整技能伤害入参类型 } from '@/@types/计算'
...
export const 完整技能伤害 = (参数: 完整技能伤害入参类型) => {
  ...
  if (判断伤害类型(计算公式计算类型.基础, 伤害计算类型标记)) {
    基础伤害 = 技能基础伤害(当前技能属性, 最终人物属性, 技能增伤?.技能系数)
  }

  // 真实伤害
  if (当前技能属性.真实伤害 && 判断伤害类型(计算公式计算类型.真实伤害, 伤害计算类型标记)) {
    基础伤害 = 基础伤害 + (当前技能属性.真实伤害 || 0)
  }

  // 全局伤害因子
  if (技能增伤?.全局伤害因子 !== 1 && 判断伤害类型(计算公式计算类型.全局伤害因子, 伤害计算类型标记)) {
    基础伤害 = 基础伤害 * 技能增伤?.全局伤害因子
  }


  // 计算伤害增加
  if (技能增伤?.通用增伤 !== 1 && 判断伤害类型(计算公式计算类型.增伤, 伤害计算类型标记)) {
    基础伤害 = 基础伤害 * 技能增伤?.通用增伤
  }

  // 计算移动状态增伤，单独乘区
  if (技能增伤?.目标移动状增伤 !== 1 && 判断伤害类型(计算公式计算类型.目标移动状增伤, 伤害计算类型标记)) {
...
const 会效后计算公式 = (
  ...
  // 非侠士增伤
  if (技能增伤.非侠增伤 !== 1 && 判断伤害类型(计算公式计算类型.非侠, 伤害计算类型标记)) {
    计算基础伤害 = 计算基础伤害 * 技能增伤.非侠增伤
  }

  // 易伤增伤
  if (技能增伤.易伤增伤 !== 1 && 判断伤害类型(计算公式计算类型.易伤, 伤害计算类型标记)) {
