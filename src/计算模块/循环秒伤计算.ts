...
export const 循环秒伤计算 = (props: 技能总伤害计算入参类型) => {
  const { 计算循环, 装备信息, 当前目标, 技能基础数据, 增益启用, 增益数据, 战斗时间, 是否郭氏计算 } =
    props
  // 总dps
  let 循环总伤 = 0
  // 每个技能的dps总和列表
  ...
  const 计算目标 = 当前目标
  const 计算属性 = 获取全能加成面板(装备信息?.装备基础属性)

  // 获取装备增益等带来的最终增益集合
  let 总增益集合: 属性加成[] = 获取装备增益(装备信息)
  ...
  if (当前技能属性) {
    ...
    if (循环?.技能增益列表?.length) {
      循环?.技能增益列表.forEach((增益) => {
        ...
        const 技能独立增益集合列表: 属性加成[] = 计算该技能下多个增益的增益集合(增益, 当前技能属性)
        if (增益.增益技能数) {
...
export const 计算技能总伤 = (
  ...
  return { 会心数量: 0, ...计算技能总伤函数(最终计算属性, 当前技能属性, 技能总数) }
}
...
export const 计算增益加成后最终计算属性 = (
  ...
  const 当前技能计算增益集合: 属性加成[] = 增益排序(总增益集合)

  console.log('当前技能计算增益集合', 当前技能计算增益集合)

  当前技能计算增益集合.forEach((增益数值信息) => {
    const 计算后对象 = 通用增益计算(增益数值信息, 最终计算属性)
    console.log('计算后对象', 计算后对象)
    最终计算属性 = {
      ...最终计算属性,
      ...计算后对象,
    }
