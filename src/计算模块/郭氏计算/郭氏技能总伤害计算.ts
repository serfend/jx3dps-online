...
export const 获取属性加成后的面板 = (最终计算属性: 最终计算属性类型) => {
  ...
  const 最终力道 = 获取郭氏加成值(最终计算属性?.最终人物属性?.力道, 最终计算属性?.郭氏力道)
  const 最终身法 = 获取郭氏加成值(最终计算属性?.最终人物属性?.身法, 最终计算属性?.郭氏身法)
  const 最终根骨 = 获取郭氏加成值(最终计算属性?.最终人物属性?.根骨, 最终计算属性?.郭氏根骨)
  const 最终元气 = 获取郭氏加成值(最终计算属性?.最终人物属性?.元气, 最终计算属性?.郭氏元气)
  ...
  // 计算主属性的面板攻击
  计算人物面板.面板攻击 = (计算人物面板.基础攻击 || 0) + INT(最终主属性 * 额外加成面板攻击)

  // 计算外功
  if (功法 === '外功') {
    // 力道-基础攻击
    计算人物面板.基础攻击 += INT(最终力道 * 基础系数.力道转换攻击)
    计算人物面板.面板攻击 += INT(最终力道 * 基础系数.力道转换攻击)
    // 力道-破防
    计算人物面板.破防等级 += INT(最终力道 * 基础系数.力道转换破防) + INT(最终主属性 * 额外加成破防)
    // 身法-会心
    计算人物面板.会心等级 += INT(最终身法 * 基础系数.身法转换会心) + INT(最终主属性 * 额外加成会心)
  }
  // 计算内功
  if (功法 === '内功') {
    // 元气-基础攻击
    计算人物面板.基础攻击 += INT(最终元气 * 基础系数.元气转换攻击)
    计算人物面板.面板攻击 += INT(最终元气 * 基础系数.元气转换攻击)
    // 元气-破防
    计算人物面板.破防等级 += INT(最终元气 * 基础系数.元气转换破防) + INT(最终主属性 * 额外加成破防)
    // 根骨-会心
    计算人物面板.会心等级 += INT(最终根骨 * 基础系数.根骨转换会心) + INT(最终主属性 * 额外加成会心)
  }
