import { 属性类型 } from '@/@types/属性'

import { 装备属性信息模型, 装备特效枚举, 装备类型枚举 } from '@/@types/装备'

const 腰带装备数据: 装备属性信息模型[] = [
  {
    id: 98484,
    uid: 215172,
    装备名称: '水川腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 15800,
    装备特效: 装备特效枚举.切糕_英雄,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3983 },
      { 属性: 属性类型.根骨, 值: 772 },
      { 属性: 属性类型.内功基础攻击, 值: 1503 },
      { 属性: 属性类型.内功会心等级, 值: 3875 },
      { 属性: 属性类型.破招值, 值: 3444 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 98485,
    uid: 215173,
    装备名称: '水泓腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 15800,
    装备特效: 装备特效枚举.切糕_英雄,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3983 },
      { 属性: 属性类型.元气, 值: 772 },
      { 属性: 属性类型.内功基础攻击, 值: 1503 },
      { 属性: 属性类型.全会心等级, 值: 3875 },
      { 属性: 属性类型.破招值, 值: 3444 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 100053,
    uid: 216981,
    装备名称: '陶然意·和时腰带',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 15700,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3957 },
      { 属性: 属性类型.根骨, 值: 719 },
      { 属性: 属性类型.内功基础攻击, 值: 1149 },
      { 属性: 属性类型.内功会心等级, 值: 3850 },
      { 属性: 属性类型.全能等级, 值: 1711 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 100052,
    uid: 216980,
    装备名称: '陶然意·星雪腰带',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 15700,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3957 },
      { 属性: 属性类型.元气, 值: 719 },
      { 属性: 属性类型.内功基础攻击, 值: 1149 },
      { 属性: 属性类型.内功会心等级, 值: 3850 },
      { 属性: 属性类型.全能等级, 值: 1711 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 100036,
    uid: 216964,
    装备名称: '陶然意·兰谷腰带',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 15700,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3957 },
      { 属性: 属性类型.元气, 值: 719 },
      { 属性: 属性类型.内功基础攻击, 值: 1149 },
      { 属性: 属性类型.内功会心等级, 值: 3850 },
      { 属性: 属性类型.全能等级, 值: 1711 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 98376,
    uid: 214880,
    装备名称: '绕城腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3932 },
      { 属性: 属性类型.根骨, 值: 763 },
      { 属性: 属性类型.内功基础攻击, 值: 1484 },
      { 属性: 属性类型.内功破防等级, 值: 3826 },
      { 属性: 属性类型.无双等级, 值: 3401 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 98377,
    uid: 214881,
    装备名称: '宫阙腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3932 },
      { 属性: 属性类型.元气, 值: 763 },
      { 属性: 属性类型.内功基础攻击, 值: 1484 },
      { 属性: 属性类型.内功破防等级, 值: 3826 },
      { 属性: 属性类型.无双等级, 值: 3401 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 98412,
    uid: 214940,
    装备名称: '情义带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3932 },
      { 属性: 属性类型.根骨, 值: 763 },
      { 属性: 属性类型.内功基础攻击, 值: 1484 },
      { 属性: 属性类型.加速等级, 值: 3826 },
      { 属性: 属性类型.破招值, 值: 3401 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98413,
    uid: 214941,
    装备名称: '良安带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3932 },
      { 属性: 属性类型.元气, 值: 763 },
      { 属性: 属性类型.内功基础攻击, 值: 1484 },
      { 属性: 属性类型.加速等级, 值: 3826 },
      { 属性: 属性类型.破招值, 值: 3401 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98442,
    uid: 215062,
    装备名称: '流送腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 15600,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3882 },
      { 属性: 属性类型.全会心等级, 值: 7546 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 98443,
    uid: 215063,
    装备名称: '伏林腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 15600,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3311 },
      { 属性: 属性类型.破招值, 值: 4251 },
      { 属性: 属性类型.内功破防等级, 值: 4464 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.破招值 }],
  },
  {
    id: 98444,
    uid: 215064,
    装备名称: '犹阁腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 15600,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3311 },
      { 属性: 属性类型.破招值, 值: 2338 },
      { 属性: 属性类型.全会心等级, 值: 4039 },
      { 属性: 属性类型.无双等级, 值: 2338 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 98448,
    uid: 215068,
    装备名称: '因其腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 15600,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3882 },
      { 属性: 属性类型.无双等级, 值: 7546 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 98466,
    uid: 215094,
    装备名称: '令上束腰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 15600,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3932 },
      { 属性: 属性类型.内功基础攻击, 值: 3197 },
      { 属性: 属性类型.内功破防等级, 值: 4039 },
      { 属性: 属性类型.无双等级, 值: 4464 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.破招值 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98467,
    uid: 215095,
    装备名称: '文成束腰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 15600,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3932 },
      { 属性: 属性类型.内功基础攻击, 值: 2683 },
      { 属性: 属性类型.破招值, 值: 4782 },
      { 属性: 属性类型.无双等级, 值: 4782 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 98586,
    uid: 215347,
    装备名称: '月纤腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3932 },
      { 属性: 属性类型.根骨, 值: 763 },
      { 属性: 属性类型.内功基础攻击, 值: 1484 },
      { 属性: 属性类型.内功会心等级, 值: 3826 },
      { 属性: 属性类型.破招值, 值: 3401 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 98587,
    uid: 215348,
    装备名称: '月迟腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3932 },
      { 属性: 属性类型.元气, 值: 763 },
      { 属性: 属性类型.内功基础攻击, 值: 1484 },
      { 属性: 属性类型.全会心等级, 值: 3826 },
      { 属性: 属性类型.破招值, 值: 3401 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 98252,
    uid: 214756,
    装备名称: '鸿辉·鹿喧腰带',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 15400,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3882 },
      { 属性: 属性类型.元气, 值: 753 },
      { 属性: 属性类型.内功基础攻击, 值: 1465 },
      { 属性: 属性类型.内功会心等级, 值: 3777 },
      { 属性: 属性类型.无双等级, 值: 3357 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98275,
    uid: 214779,
    装备名称: '鸿辉·梦辰腰带',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 15400,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3882 },
      { 属性: 属性类型.元气, 值: 753 },
      { 属性: 属性类型.内功基础攻击, 值: 1465 },
      { 属性: 属性类型.内功会心等级, 值: 3777 },
      { 属性: 属性类型.无双等级, 值: 3357 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98276,
    uid: 214780,
    装备名称: '鸿辉·白林腰带',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 15400,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3882 },
      { 属性: 属性类型.根骨, 值: 753 },
      { 属性: 属性类型.内功基础攻击, 值: 1465 },
      { 属性: 属性类型.内功会心等级, 值: 3777 },
      { 属性: 属性类型.无双等级, 值: 3357 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96370,
    uid: 212095,
    装备名称: '风轻腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 14150,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3567 },
      { 属性: 属性类型.根骨, 值: 692 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.内功会心等级, 值: 3470 },
      { 属性: 属性类型.破招值, 值: 3085 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 96371,
    uid: 212096,
    装备名称: '风绫腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 14150,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3567 },
      { 属性: 属性类型.元气, 值: 692 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.全会心等级, 值: 3470 },
      { 属性: 属性类型.破招值, 值: 3085 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 99910,
    uid: 216964,
    装备名称: '陶然意·兰谷腰带',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 14050,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3542 },
      { 属性: 属性类型.元气, 值: 644 },
      { 属性: 属性类型.内功基础攻击, 值: 1028 },
      { 属性: 属性类型.内功会心等级, 值: 3446 },
      { 属性: 属性类型.全能等级, 值: 1531 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 99926,
    uid: 216980,
    装备名称: '陶然意·星雪腰带',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 14050,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3542 },
      { 属性: 属性类型.元气, 值: 644 },
      { 属性: 属性类型.内功基础攻击, 值: 1028 },
      { 属性: 属性类型.内功会心等级, 值: 3446 },
      { 属性: 属性类型.全能等级, 值: 1531 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 99927,
    uid: 216981,
    装备名称: '陶然意·和时腰带',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 14050,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3542 },
      { 属性: 属性类型.根骨, 值: 644 },
      { 属性: 属性类型.内功基础攻击, 值: 1028 },
      { 属性: 属性类型.内功会心等级, 值: 3446 },
      { 属性: 属性类型.全能等级, 值: 1531 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 96262,
    uid: 211803,
    装备名称: '扬英腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.根骨, 值: 682 },
      { 属性: 属性类型.内功基础攻击, 值: 1327 },
      { 属性: 属性类型.内功破防等级, 值: 3421 },
      { 属性: 属性类型.无双等级, 值: 3041 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 96263,
    uid: 211804,
    装备名称: '开颐腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.元气, 值: 682 },
      { 属性: 属性类型.内功基础攻击, 值: 1327 },
      { 属性: 属性类型.内功破防等级, 值: 3421 },
      { 属性: 属性类型.无双等级, 值: 3041 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 96298,
    uid: 211863,
    装备名称: '寒绡带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.根骨, 值: 682 },
      { 属性: 属性类型.内功基础攻击, 值: 1327 },
      { 属性: 属性类型.加速等级, 值: 3421 },
      { 属性: 属性类型.破招值, 值: 3041 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96299,
    uid: 211864,
    装备名称: '壑云带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.元气, 值: 682 },
      { 属性: 属性类型.内功基础攻击, 值: 1327 },
      { 属性: 属性类型.加速等级, 值: 3421 },
      { 属性: 属性类型.破招值, 值: 3041 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96328,
    uid: 211985,
    装备名称: '挽星腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 13950,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3472 },
      { 属性: 属性类型.全会心等级, 值: 6748 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 96329,
    uid: 211986,
    装备名称: '芜云腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 13950,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 2961 },
      { 属性: 属性类型.破招值, 值: 3801 },
      { 属性: 属性类型.内功破防等级, 值: 3991 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.破招值 }],
  },
  {
    id: 96330,
    uid: 211987,
    装备名称: '东珞腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 13950,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 2961 },
      { 属性: 属性类型.破招值, 值: 2091 },
      { 属性: 属性类型.全会心等级, 值: 3611 },
      { 属性: 属性类型.无双等级, 值: 2091 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96334,
    uid: 211991,
    装备名称: '书鸢腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 13950,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3472 },
      { 属性: 属性类型.无双等级, 值: 6748 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96352,
    uid: 212017,
    装备名称: '楼满束腰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 13950,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.内功基础攻击, 值: 2859 },
      { 属性: 属性类型.内功破防等级, 值: 3611 },
      { 属性: 属性类型.无双等级, 值: 3991 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.破招值 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96353,
    uid: 212018,
    装备名称: '舒念束腰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 13950,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.内功基础攻击, 值: 2399 },
      { 属性: 属性类型.破招值, 值: 4277 },
      { 属性: 属性类型.无双等级, 值: 4277 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96480,
    uid: 212284,
    装备名称: '泉合腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.根骨, 值: 682 },
      { 属性: 属性类型.内功基础攻击, 值: 1327 },
      { 属性: 属性类型.内功会心等级, 值: 3421 },
      { 属性: 属性类型.破招值, 值: 3041 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 96481,
    uid: 212285,
    装备名称: '泉麓腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.元气, 值: 682 },
      { 属性: 属性类型.内功基础攻击, 值: 1327 },
      { 属性: 属性类型.全会心等级, 值: 3421 },
      { 属性: 属性类型.破招值, 值: 3041 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 98190,
    uid: 214645,
    装备名称: '危音带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.根骨, 值: 682 },
      { 属性: 属性类型.内功基础攻击, 值: 1327 },
      { 属性: 属性类型.内功会心等级, 值: 3421 },
      { 属性: 属性类型.无双等级, 值: 3041 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98191,
    uid: 214646,
    装备名称: '危影带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.元气, 值: 682 },
      { 属性: 属性类型.内功基础攻击, 值: 1327 },
      { 属性: 属性类型.全会心等级, 值: 3421 },
      { 属性: 属性类型.无双等级, 值: 3041 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98682,
    uid: 215544,
    装备名称: '东方日出·所适腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.根骨, 值: 682 },
      { 属性: 属性类型.内功基础攻击, 值: 1327 },
      { 属性: 属性类型.内功破防等级, 值: 3421 },
      { 属性: 属性类型.破招值, 值: 3041 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 98683,
    uid: 215545,
    装备名称: '东方日出·当楼腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3516 },
      { 属性: 属性类型.元气, 值: 682 },
      { 属性: 属性类型.内功基础攻击, 值: 1327 },
      { 属性: 属性类型.内功破防等级, 值: 3421 },
      { 属性: 属性类型.破招值, 值: 3041 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 96138,
    uid: 211679,
    装备名称: '灵源·月胧腰带',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 13750,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3466 },
      { 属性: 属性类型.元气, 值: 672 },
      { 属性: 属性类型.内功基础攻击, 值: 1308 },
      { 属性: 属性类型.内功会心等级, 值: 3372 },
      { 属性: 属性类型.无双等级, 值: 2998 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96161,
    uid: 211702,
    装备名称: '灵源·沉辉腰带',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 13750,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3466 },
      { 属性: 属性类型.元气, 值: 672 },
      { 属性: 属性类型.内功基础攻击, 值: 1308 },
      { 属性: 属性类型.内功会心等级, 值: 3372 },
      { 属性: 属性类型.无双等级, 值: 2998 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96162,
    uid: 211703,
    装备名称: '灵源·采芳腰带',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 13750,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3466 },
      { 属性: 属性类型.根骨, 值: 672 },
      { 属性: 属性类型.内功基础攻击, 值: 1308 },
      { 属性: 属性类型.内功会心等级, 值: 3372 },
      { 属性: 属性类型.无双等级, 值: 2998 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98166,
    uid: 214590,
    装备名称: '寻踪觅宝·折月腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13750,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3466 },
      { 属性: 属性类型.根骨, 值: 672 },
      { 属性: 属性类型.内功基础攻击, 值: 1308 },
      { 属性: 属性类型.加速等级, 值: 3372 },
      { 属性: 属性类型.破招值, 值: 2998 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98167,
    uid: 214591,
    装备名称: '寻踪觅宝·星辉腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13750,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3466 },
      { 属性: 属性类型.元气, 值: 672 },
      { 属性: 属性类型.内功基础攻击, 值: 1308 },
      { 属性: 属性类型.加速等级, 值: 3372 },
      { 属性: 属性类型.破招值, 值: 2998 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94467,
    uid: 208161,
    装备名称: '雪满腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12600,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3176 },
      { 属性: 属性类型.根骨, 值: 616 },
      { 属性: 属性类型.内功基础攻击, 值: 1199 },
      { 属性: 属性类型.内功会心等级, 值: 3090 },
      { 属性: 属性类型.破招值, 值: 2747 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 94468,
    uid: 208162,
    装备名称: '雪洁腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12600,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3176 },
      { 属性: 属性类型.元气, 值: 616 },
      { 属性: 属性类型.内功基础攻击, 值: 1199 },
      { 属性: 属性类型.全会心等级, 值: 3090 },
      { 属性: 属性类型.破招值, 值: 2747 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 97770,
    uid: 210972,
    装备名称: '碧月腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3176 },
      { 属性: 属性类型.根骨, 值: 616 },
      { 属性: 属性类型.内功基础攻击, 值: 1199 },
      { 属性: 属性类型.内功破防等级, 值: 3090 },
      { 属性: 属性类型.破招值, 值: 2747 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 97771,
    uid: 210973,
    装备名称: '幽川腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3176 },
      { 属性: 属性类型.元气, 值: 616 },
      { 属性: 属性类型.内功基础攻击, 值: 1199 },
      { 属性: 属性类型.内功破防等级, 值: 3090 },
      { 属性: 属性类型.破招值, 值: 2747 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 94371,
    uid: 207887,
    装备名称: '池泓腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.根骨, 值: 609 },
      { 属性: 属性类型.内功基础攻击, 值: 1185 },
      { 属性: 属性类型.内功破防等级, 值: 3053 },
      { 属性: 属性类型.无双等级, 值: 2714 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 94372,
    uid: 207888,
    装备名称: '椴微腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.元气, 值: 609 },
      { 属性: 属性类型.内功基础攻击, 值: 1185 },
      { 属性: 属性类型.内功破防等级, 值: 3053 },
      { 属性: 属性类型.无双等级, 值: 2714 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 94407,
    uid: 207947,
    装备名称: '朝华带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.根骨, 值: 609 },
      { 属性: 属性类型.内功基础攻击, 值: 1185 },
      { 属性: 属性类型.加速等级, 值: 3053 },
      { 属性: 属性类型.破招值, 值: 2714 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94408,
    uid: 207948,
    装备名称: '沁渡带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.元气, 值: 609 },
      { 属性: 属性类型.内功基础攻击, 值: 1185 },
      { 属性: 属性类型.加速等级, 值: 3053 },
      { 属性: 属性类型.破招值, 值: 2714 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94437,
    uid: 208067,
    装备名称: '栗烈腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 12450,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.内功基础攻击, 值: 3098 },
      { 属性: 属性类型.无双等级, 值: 5683 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 94438,
    uid: 208068,
    装备名称: '百汩腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 12450,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.内功基础攻击, 值: 2552 },
      { 属性: 属性类型.破招值, 值: 3562 },
      { 属性: 属性类型.内功破防等级, 值: 3223 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94439,
    uid: 208069,
    装备名称: '鸿北腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 12450,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.内功基础攻击, 值: 2187 },
      { 属性: 属性类型.全会心等级, 值: 2205 },
      { 属性: 属性类型.无双等级, 值: 5428 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.破招值 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94443,
    uid: 208073,
    装备名称: '寸碧腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 12450,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.内功基础攻击, 值: 2552 },
      { 属性: 属性类型.内功破防等级, 值: 3223 },
      { 属性: 属性类型.无双等级, 值: 3562 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.破招值 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94569,
    uid: 208335,
    装备名称: '湖寂腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.根骨, 值: 609 },
      { 属性: 属性类型.内功基础攻击, 值: 1185 },
      { 属性: 属性类型.内功会心等级, 值: 3053 },
      { 属性: 属性类型.破招值, 值: 2714 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 94570,
    uid: 208336,
    装备名称: '湖烟腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.元气, 值: 609 },
      { 属性: 属性类型.内功基础攻击, 值: 1185 },
      { 属性: 属性类型.全会心等级, 值: 3053 },
      { 属性: 属性类型.破招值, 值: 2714 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 96576,
    uid: 212481,
    装备名称: '西风北啸·音书腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.根骨, 值: 609 },
      { 属性: 属性类型.内功基础攻击, 值: 1185 },
      { 属性: 属性类型.内功破防等级, 值: 3053 },
      { 属性: 属性类型.破招值, 值: 2714 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 96577,
    uid: 212482,
    装备名称: '西风北啸·离声腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3138 },
      { 属性: 属性类型.元气, 值: 609 },
      { 属性: 属性类型.内功基础攻击, 值: 1185 },
      { 属性: 属性类型.内功破防等级, 值: 3053 },
      { 属性: 属性类型.破招值, 值: 2714 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 90513,
    uid: 189005,
    装备名称: '临罡腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12400,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3126 },
      { 属性: 属性类型.根骨, 值: 606 },
      { 属性: 属性类型.内功基础攻击, 值: 1180 },
      { 属性: 属性类型.内功破防等级, 值: 3041 },
      { 属性: 属性类型.无双等级, 值: 2703 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 90514,
    uid: 189006,
    装备名称: '临初腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12400,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3126 },
      { 属性: 属性类型.元气, 值: 606 },
      { 属性: 属性类型.内功基础攻击, 值: 1180 },
      { 属性: 属性类型.内功破防等级, 值: 3041 },
      { 属性: 属性类型.无双等级, 值: 2703 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 90333,
    uid: 188591,
    装备名称: '德襄带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.破招值, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 90334,
    uid: 188592,
    装备名称: '烟琐带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.破招值, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 90369,
    uid: 188681,
    装备名称: '擒雨带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功破防等级, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 90370,
    uid: 188682,
    装备名称: '语阔带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功破防等级, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 90405,
    uid: 188771,
    装备名称: '忆安带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.加速等级, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 90406,
    uid: 188772,
    装备名称: '忆惜带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.加速等级, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 90537,
    uid: 189053,
    装备名称: '故云腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功会心等级, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90538,
    uid: 189054,
    装备名称: '庭澜腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.全会心等级, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90573,
    uid: 189143,
    装备名称: '途南腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功破防等级, 值: 3017 },
      { 属性: 属性类型.破招值, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 90574,
    uid: 189144,
    装备名称: '花霭腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功破防等级, 值: 3017 },
      { 属性: 属性类型.破招值, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 90609,
    uid: 189233,
    装备名称: '剪桐腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功破防等级, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 90610,
    uid: 189234,
    装备名称: '故芳腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功破防等级, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 90645,
    uid: 189323,
    装备名称: '谨峰腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.加速等级, 值: 3017 },
      { 属性: 属性类型.破招值, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90646,
    uid: 189324,
    装备名称: '藏峦腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.加速等级, 值: 3017 },
      { 属性: 属性类型.破招值, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94251,
    uid: 207767,
    装备名称: '濯心·松声腰带',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 12300,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功会心等级, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94274,
    uid: 207790,
    装备名称: '濯心·天尘腰带',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 12300,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功会心等级, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94275,
    uid: 207791,
    装备名称: '濯心·采青腰带',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 12300,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功会心等级, 值: 3017 },
      { 属性: 属性类型.无双等级, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96082,
    uid: 211592,
    装备名称: '寻踪觅宝·拂雪腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.加速等级, 值: 3017 },
      { 属性: 属性类型.破招值, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96083,
    uid: 211593,
    装备名称: '寻踪觅宝·泻雨腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.加速等级, 值: 3017 },
      { 属性: 属性类型.破招值, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98100,
    uid: 214293,
    装备名称: '翡翠御厨腰带·火候',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 12300,
    装备特效: 装备特效枚举.冬至套装,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功会心等级, 值: 3017 },
      { 属性: 属性类型.破招值, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 98123,
    uid: 214316,
    装备名称: '寻龙御厨腰带·火候',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 12300,
    装备特效: 装备特效枚举.冬至套装,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功会心等级, 值: 3017 },
      { 属性: 属性类型.破招值, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 98124,
    uid: 214317,
    装备名称: '迎新御厨腰带·火候',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 12300,
    装备特效: 装备特效枚举.冬至套装,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 601 },
      { 属性: 属性类型.内功基础攻击, 值: 1170 },
      { 属性: 属性类型.内功会心等级, 值: 3017 },
      { 属性: 属性类型.破招值, 值: 2681 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 100339,
    uid: 217814,
    装备名称: '若寓带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 564 },
      { 属性: 属性类型.内功基础攻击, 值: 900 },
      { 属性: 属性类型.内功破防等级, 值: 3017 },
      { 属性: 属性类型.全能等级, 值: 1341 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 100338,
    uid: 217813,
    装备名称: '越风带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 564 },
      { 属性: 属性类型.内功基础攻击, 值: 900 },
      { 属性: 属性类型.内功破防等级, 值: 3017 },
      { 属性: 属性类型.全能等级, 值: 1341 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 100303,
    uid: 217723,
    装备名称: '踏江带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.元气, 值: 564 },
      { 属性: 属性类型.内功基础攻击, 值: 900 },
      { 属性: 属性类型.全会心等级, 值: 3017 },
      { 属性: 属性类型.全能等级, 值: 1341 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 100302,
    uid: 217722,
    装备名称: '承康带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3100 },
      { 属性: 属性类型.根骨, 值: 564 },
      { 属性: 属性类型.内功基础攻击, 值: 900 },
      { 属性: 属性类型.内功会心等级, 值: 3017 },
      { 属性: 属性类型.全能等级, 值: 1341 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 97764,
    uid: 210960,
    装备名称: '碧月腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11900,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3000 },
      { 属性: 属性类型.根骨, 值: 582 },
      { 属性: 属性类型.内功基础攻击, 值: 1132 },
      { 属性: 属性类型.内功破防等级, 值: 2919 },
      { 属性: 属性类型.破招值, 值: 2594 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 97765,
    uid: 210961,
    装备名称: '幽川腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11900,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3000 },
      { 属性: 属性类型.元气, 值: 582 },
      { 属性: 属性类型.内功基础攻击, 值: 1132 },
      { 属性: 属性类型.内功破防等级, 值: 2919 },
      { 属性: 属性类型.破招值, 值: 2594 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 90489,
    uid: 188957,
    装备名称: '临罡腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11800,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2974 },
      { 属性: 属性类型.根骨, 值: 577 },
      { 属性: 属性类型.内功基础攻击, 值: 1123 },
      { 属性: 属性类型.内功破防等级, 值: 2894 },
      { 属性: 属性类型.无双等级, 值: 2572 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 90490,
    uid: 188958,
    装备名称: '临初腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11800,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2974 },
      { 属性: 属性类型.元气, 值: 577 },
      { 属性: 属性类型.内功基础攻击, 值: 1123 },
      { 属性: 属性类型.内功破防等级, 值: 2894 },
      { 属性: 属性类型.无双等级, 值: 2572 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 93487,
    uid: 204761,
    装备名称: '月阑带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2899 },
      { 属性: 属性类型.根骨, 值: 562 },
      { 属性: 属性类型.内功基础攻击, 值: 1094 },
      { 属性: 属性类型.内功破防等级, 值: 2820 },
      { 属性: 属性类型.破招值, 值: 2507 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 93488,
    uid: 204762,
    装备名称: '轻船带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2899 },
      { 属性: 属性类型.元气, 值: 562 },
      { 属性: 属性类型.内功基础攻击, 值: 1094 },
      { 属性: 属性类型.内功破防等级, 值: 2820 },
      { 属性: 属性类型.破招值, 值: 2507 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 91295,
    uid: 200465,
    装备名称: '暮漠腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11300,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2848 },
      { 属性: 属性类型.根骨, 值: 552 },
      { 属性: 属性类型.内功基础攻击, 值: 1075 },
      { 属性: 属性类型.内功会心等级, 值: 2771 },
      { 属性: 属性类型.破招值, 值: 2463 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 91296,
    uid: 200466,
    装备名称: '暮盈腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11300,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2848 },
      { 属性: 属性类型.元气, 值: 552 },
      { 属性: 属性类型.内功基础攻击, 值: 1075 },
      { 属性: 属性类型.全会心等级, 值: 2771 },
      { 属性: 属性类型.破招值, 值: 2463 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 95823,
    uid: 210972,
    装备名称: '碧月腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2848 },
      { 属性: 属性类型.根骨, 值: 552 },
      { 属性: 属性类型.内功基础攻击, 值: 1075 },
      { 属性: 属性类型.内功破防等级, 值: 2771 },
      { 属性: 属性类型.破招值, 值: 2463 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 95824,
    uid: 210973,
    装备名称: '幽川腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2848 },
      { 属性: 属性类型.元气, 值: 552 },
      { 属性: 属性类型.内功基础攻击, 值: 1075 },
      { 属性: 属性类型.内功破防等级, 值: 2771 },
      { 属性: 属性类型.破招值, 值: 2463 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 90465,
    uid: 188909,
    装备名称: '临罡腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11200,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2823 },
      { 属性: 属性类型.根骨, 值: 547 },
      { 属性: 属性类型.内功基础攻击, 值: 1066 },
      { 属性: 属性类型.内功破防等级, 值: 2747 },
      { 属性: 属性类型.无双等级, 值: 2442 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 90466,
    uid: 188910,
    装备名称: '临初腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11200,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2823 },
      { 属性: 属性类型.元气, 值: 547 },
      { 属性: 属性类型.内功基础攻击, 值: 1066 },
      { 属性: 属性类型.内功破防等级, 值: 2747 },
      { 属性: 属性类型.无双等级, 值: 2442 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 97758,
    uid: 210948,
    装备名称: '碧月腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11200,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2823 },
      { 属性: 属性类型.根骨, 值: 547 },
      { 属性: 属性类型.内功基础攻击, 值: 1066 },
      { 属性: 属性类型.内功破防等级, 值: 2747 },
      { 属性: 属性类型.破招值, 值: 2442 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 97759,
    uid: 210949,
    装备名称: '幽川腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11200,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2823 },
      { 属性: 属性类型.元气, 值: 547 },
      { 属性: 属性类型.内功基础攻击, 值: 1066 },
      { 属性: 属性类型.内功破防等级, 值: 2747 },
      { 属性: 属性类型.破招值, 值: 2442 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 90958,
    uid: 189858,
    装备名称: '湘沅腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2811 },
      { 属性: 属性类型.根骨, 值: 545 },
      { 属性: 属性类型.内功基础攻击, 值: 1061 },
      { 属性: 属性类型.内功会心等级, 值: 2735 },
      { 属性: 属性类型.破招值, 值: 2431 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 90959,
    uid: 189859,
    装备名称: '湘崖腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2811 },
      { 属性: 属性类型.元气, 值: 545 },
      { 属性: 属性类型.内功基础攻击, 值: 1061 },
      { 属性: 属性类型.全会心等级, 值: 2735 },
      { 属性: 属性类型.破招值, 值: 2431 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 91199,
    uid: 200191,
    装备名称: '晓菡带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2811 },
      { 属性: 属性类型.根骨, 值: 545 },
      { 属性: 属性类型.内功基础攻击, 值: 1061 },
      { 属性: 属性类型.内功破防等级, 值: 2735 },
      { 属性: 属性类型.无双等级, 值: 2431 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 91200,
    uid: 200192,
    装备名称: '凝青带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2811 },
      { 属性: 属性类型.元气, 值: 545 },
      { 属性: 属性类型.内功基础攻击, 值: 1061 },
      { 属性: 属性类型.内功破防等级, 值: 2735 },
      { 属性: 属性类型.无双等级, 值: 2431 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 91235,
    uid: 200251,
    装备名称: '思菱腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2811 },
      { 属性: 属性类型.根骨, 值: 545 },
      { 属性: 属性类型.内功基础攻击, 值: 1061 },
      { 属性: 属性类型.加速等级, 值: 2735 },
      { 属性: 属性类型.破招值, 值: 2431 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91236,
    uid: 200252,
    装备名称: '白萱腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2811 },
      { 属性: 属性类型.元气, 值: 545 },
      { 属性: 属性类型.内功基础攻击, 值: 1061 },
      { 属性: 属性类型.加速等级, 值: 2735 },
      { 属性: 属性类型.破招值, 值: 2431 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91265,
    uid: 200377,
    装备名称: '锦字腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 11150,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2811 },
      { 属性: 属性类型.内功基础攻击, 值: 2775 },
      { 属性: 属性类型.无双等级, 值: 4786 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 91266,
    uid: 200378,
    装备名称: '重桦腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 11150,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2811 },
      { 属性: 属性类型.内功基础攻击, 值: 2285 },
      { 属性: 属性类型.破招值, 值: 2886 },
      { 属性: 属性类型.内功破防等级, 值: 2886 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91267,
    uid: 200379,
    装备名称: '六叶腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 11150,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2811 },
      { 属性: 属性类型.内功基础攻击, 值: 1959 },
      { 属性: 属性类型.全会心等级, 值: 1671 },
      { 属性: 属性类型.无双等级, 值: 4861 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.破招值 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91271,
    uid: 200383,
    装备名称: '樵隐腰带',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 11150,
    装备类型: 装备类型枚举.副本精简,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2811 },
      { 属性: 属性类型.内功基础攻击, 值: 1959 },
      { 属性: 属性类型.内功破防等级, 值: 1671 },
      { 属性: 属性类型.无双等级, 值: 4861 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.破招值 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90261,
    uid: 188411,
    装备名称: '岭深带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.根骨, 值: 538 },
      { 属性: 属性类型.内功基础攻击, 值: 1047 },
      { 属性: 属性类型.内功破防等级, 值: 2698 },
      { 属性: 属性类型.破招值, 值: 2398 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 90262,
    uid: 188412,
    装备名称: '散幽带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.元气, 值: 538 },
      { 属性: 属性类型.内功基础攻击, 值: 1047 },
      { 属性: 属性类型.内功破防等级, 值: 2698 },
      { 属性: 属性类型.破招值, 值: 2398 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 91079,
    uid: 200071,
    装备名称: '揽江·落华腰带',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 11000,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.元气, 值: 538 },
      { 属性: 属性类型.内功基础攻击, 值: 1047 },
      { 属性: 属性类型.内功会心等级, 值: 2698 },
      { 属性: 属性类型.无双等级, 值: 2398 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91102,
    uid: 200094,
    装备名称: '揽江·玉玑腰带',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 11000,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.元气, 值: 538 },
      { 属性: 属性类型.内功基础攻击, 值: 1047 },
      { 属性: 属性类型.内功会心等级, 值: 2698 },
      { 属性: 属性类型.无双等级, 值: 2398 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91103,
    uid: 200095,
    装备名称: '揽江·柳颦腰带',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 11000,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.根骨, 值: 538 },
      { 属性: 属性类型.内功基础攻击, 值: 1047 },
      { 属性: 属性类型.内功会心等级, 值: 2698 },
      { 属性: 属性类型.无双等级, 值: 2398 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94196,
    uid: 207682,
    装备名称: '寻踪觅宝·花狸腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.根骨, 值: 538 },
      { 属性: 属性类型.内功基础攻击, 值: 1047 },
      { 属性: 属性类型.加速等级, 值: 2698 },
      { 属性: 属性类型.破招值, 值: 2398 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94197,
    uid: 207683,
    装备名称: '寻踪觅宝·细雨腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.元气, 值: 538 },
      { 属性: 属性类型.内功基础攻击, 值: 1047 },
      { 属性: 属性类型.加速等级, 值: 2698 },
      { 属性: 属性类型.破招值, 值: 2398 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94665,
    uid: 208547,
    装备名称: '风销残烟·幽吹腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.根骨, 值: 538 },
      { 属性: 属性类型.内功基础攻击, 值: 1047 },
      { 属性: 属性类型.内功破防等级, 值: 2698 },
      { 属性: 属性类型.破招值, 值: 2398 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 94666,
    uid: 208548,
    装备名称: '风销残烟·樗闲腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.元气, 值: 538 },
      { 属性: 属性类型.内功基础攻击, 值: 1047 },
      { 属性: 属性类型.内功破防等级, 值: 2698 },
      { 属性: 属性类型.破招值, 值: 2398 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 100266,
    uid: 217631,
    装备名称: '丹风腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.根骨, 值: 538 },
      { 属性: 属性类型.内功基础攻击, 值: 1047 },
      { 属性: 属性类型.内功破防等级, 值: 2698 },
      { 属性: 属性类型.破招值, 值: 2398 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 100267,
    uid: 217632,
    装备名称: '神逸腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.元气, 值: 538 },
      { 属性: 属性类型.内功基础攻击, 值: 1047 },
      { 属性: 属性类型.内功破防等级, 值: 2698 },
      { 属性: 属性类型.破招值, 值: 2398 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 100465,
    uid: 218116,
    装备名称: '神逸腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.元气, 值: 504 },
      { 属性: 属性类型.内功基础攻击, 值: 805 },
      { 属性: 属性类型.内功破防等级, 值: 2698 },
      { 属性: 属性类型.全能等级, 值: 1199 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 100464,
    uid: 218115,
    装备名称: '丹风腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2773 },
      { 属性: 属性类型.根骨, 值: 504 },
      { 属性: 属性类型.内功基础攻击, 值: 805 },
      { 属性: 属性类型.内功破防等级, 值: 2698 },
      { 属性: 属性类型.全能等级, 值: 1199 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90225,
    uid: 188321,
    装备名称: '北池护腰',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10800,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2722 },
      { 属性: 属性类型.根骨, 值: 660 },
      { 属性: 属性类型.内功基础攻击, 值: 1581 },
      { 属性: 属性类型.内功会心等级, 值: 3384 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.破招值 }],
  },
  {
    id: 90226,
    uid: 188322,
    装备名称: '澹风护腰',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10800,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2722 },
      { 属性: 属性类型.元气, 值: 660 },
      { 属性: 属性类型.内功基础攻击, 值: 1581 },
      { 属性: 属性类型.全会心等级, 值: 3384 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.破招值 }],
  },
  {
    id: 95817,
    uid: 210960,
    装备名称: '碧月腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10700,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2697 },
      { 属性: 属性类型.根骨, 值: 523 },
      { 属性: 属性类型.内功基础攻击, 值: 1018 },
      { 属性: 属性类型.内功破防等级, 值: 2624 },
      { 属性: 属性类型.破招值, 值: 2333 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 95818,
    uid: 210961,
    装备名称: '幽川腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10700,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2697 },
      { 属性: 属性类型.元气, 值: 523 },
      { 属性: 属性类型.内功基础攻击, 值: 1018 },
      { 属性: 属性类型.内功破防等级, 值: 2624 },
      { 属性: 属性类型.破招值, 值: 2333 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 90441,
    uid: 188861,
    装备名称: '临罡腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2672 },
      { 属性: 属性类型.根骨, 值: 518 },
      { 属性: 属性类型.内功基础攻击, 值: 1009 },
      { 属性: 属性类型.内功破防等级, 值: 2600 },
      { 属性: 属性类型.无双等级, 值: 2311 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 90442,
    uid: 188862,
    装备名称: '临初腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2672 },
      { 属性: 属性类型.元气, 值: 518 },
      { 属性: 属性类型.内功基础攻击, 值: 1009 },
      { 属性: 属性类型.内功破防等级, 值: 2600 },
      { 属性: 属性类型.无双等级, 值: 2311 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 97752,
    uid: 210936,
    装备名称: '碧月腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2647 },
      { 属性: 属性类型.根骨, 值: 513 },
      { 属性: 属性类型.内功基础攻击, 值: 999 },
      { 属性: 属性类型.内功破防等级, 值: 2575 },
      { 属性: 属性类型.破招值, 值: 2289 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 97753,
    uid: 210937,
    装备名称: '幽川腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2647 },
      { 属性: 属性类型.元气, 值: 513 },
      { 属性: 属性类型.内功基础攻击, 值: 999 },
      { 属性: 属性类型.内功破防等级, 值: 2575 },
      { 属性: 属性类型.破招值, 值: 2289 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 36604,
    uid: 119935,
    装备名称: '半夏腰带',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2647 },
      { 属性: 属性类型.元气, 值: 481 },
      { 属性: 属性类型.内功基础攻击, 值: 769 },
      { 属性: 属性类型.内功破防等级, 值: 2575 },
      { 属性: 属性类型.全能等级, 值: 1145 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 52880,
    uid: 171361,
    装备名称: '勉学带',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2647 },
      { 属性: 属性类型.元气, 值: 481 },
      { 属性: 属性类型.内功基础攻击, 值: 769 },
      { 属性: 属性类型.内功破防等级, 值: 2575 },
      { 属性: 属性类型.全能等级, 值: 1145 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 57202,
    uid: 181717,
    装备名称: '归林带',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2647 },
      { 属性: 属性类型.根骨, 值: 481 },
      { 属性: 属性类型.内功基础攻击, 值: 769 },
      { 属性: 属性类型.内功破防等级, 值: 2575 },
      { 属性: 属性类型.全能等级, 值: 1145 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90153,
    uid: 188141,
    装备名称: '凭心腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2647 },
      { 属性: 属性类型.根骨, 值: 481 },
      { 属性: 属性类型.内功基础攻击, 值: 769 },
      { 属性: 属性类型.内功破防等级, 值: 2575 },
      { 属性: 属性类型.全能等级, 值: 1145 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90154,
    uid: 188142,
    装备名称: '无竟腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2647 },
      { 属性: 属性类型.元气, 值: 481 },
      { 属性: 属性类型.内功基础攻击, 值: 769 },
      { 属性: 属性类型.内功破防等级, 值: 2575 },
      { 属性: 属性类型.全能等级, 值: 1145 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 94160,
    uid: 207598,
    装备名称: '虚香带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10400,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2621 },
      { 属性: 属性类型.根骨, 值: 508 },
      { 属性: 属性类型.内功基础攻击, 值: 990 },
      { 属性: 属性类型.内功会心等级, 值: 2551 },
      { 属性: 属性类型.无双等级, 值: 2267 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94161,
    uid: 207599,
    装备名称: '虚缘带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10400,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2621 },
      { 属性: 属性类型.元气, 值: 508 },
      { 属性: 属性类型.内功基础攻击, 值: 990 },
      { 属性: 属性类型.全会心等级, 值: 2551 },
      { 属性: 属性类型.无双等级, 值: 2267 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90886,
    uid: 189702,
    装备名称: '持溯带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2558 },
      { 属性: 属性类型.根骨, 值: 496 },
      { 属性: 属性类型.内功基础攻击, 值: 966 },
      { 属性: 属性类型.内功破防等级, 值: 2489 },
      { 属性: 属性类型.无双等级, 值: 2213 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 90887,
    uid: 189703,
    装备名称: '青霍带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2558 },
      { 属性: 属性类型.元气, 值: 496 },
      { 属性: 属性类型.内功基础攻击, 值: 966 },
      { 属性: 属性类型.内功破防等级, 值: 2489 },
      { 属性: 属性类型.无双等级, 值: 2213 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 90922,
    uid: 189762,
    装备名称: '末闻腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2558 },
      { 属性: 属性类型.根骨, 值: 496 },
      { 属性: 属性类型.内功基础攻击, 值: 966 },
      { 属性: 属性类型.加速等级, 值: 2489 },
      { 属性: 属性类型.破招值, 值: 2213 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90923,
    uid: 189763,
    装备名称: '啸影腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2558 },
      { 属性: 属性类型.元气, 值: 496 },
      { 属性: 属性类型.内功基础攻击, 值: 966 },
      { 属性: 属性类型.加速等级, 值: 2489 },
      { 属性: 属性类型.破招值, 值: 2213 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 95811,
    uid: 210948,
    装备名称: '碧月腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10100,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2546 },
      { 属性: 属性类型.根骨, 值: 494 },
      { 属性: 属性类型.内功基础攻击, 值: 961 },
      { 属性: 属性类型.内功破防等级, 值: 2477 },
      { 属性: 属性类型.破招值, 值: 2202 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 95812,
    uid: 210949,
    装备名称: '幽川腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10100,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2546 },
      { 属性: 属性类型.元气, 值: 494 },
      { 属性: 属性类型.内功基础攻击, 值: 961 },
      { 属性: 属性类型.内功破防等级, 值: 2477 },
      { 属性: 属性类型.破招值, 值: 2202 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 90711,
    uid: 189497,
    装备名称: '寻踪觅宝·君溪腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2521 },
      { 属性: 属性类型.根骨, 值: 489 },
      { 属性: 属性类型.内功基础攻击, 值: 952 },
      { 属性: 属性类型.加速等级, 值: 2453 },
      { 属性: 属性类型.破招值, 值: 2180 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90712,
    uid: 189498,
    装备名称: '寻踪觅宝·莫南腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2521 },
      { 属性: 属性类型.元气, 值: 489 },
      { 属性: 属性类型.内功基础攻击, 值: 952 },
      { 属性: 属性类型.加速等级, 值: 2453 },
      { 属性: 属性类型.破招值, 值: 2180 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90766,
    uid: 189582,
    装备名称: '展锋·看妆腰带',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 10000,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2521 },
      { 属性: 属性类型.元气, 值: 489 },
      { 属性: 属性类型.内功基础攻击, 值: 952 },
      { 属性: 属性类型.内功会心等级, 值: 2453 },
      { 属性: 属性类型.无双等级, 值: 2180 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90789,
    uid: 189605,
    装备名称: '展锋·晓河护腰',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 10000,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2521 },
      { 属性: 属性类型.元气, 值: 489 },
      { 属性: 属性类型.内功基础攻击, 值: 952 },
      { 属性: 属性类型.内功会心等级, 值: 2453 },
      { 属性: 属性类型.无双等级, 值: 2180 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90790,
    uid: 189606,
    装备名称: '展锋·待芳腰带',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 10000,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2521 },
      { 属性: 属性类型.根骨, 值: 489 },
      { 属性: 属性类型.内功基础攻击, 值: 952 },
      { 属性: 属性类型.内功会心等级, 值: 2453 },
      { 属性: 属性类型.无双等级, 值: 2180 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 92491,
    uid: 202687,
    装备名称: '沉龙荡波·漠天腰带',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2521 },
      { 属性: 属性类型.根骨, 值: 489 },
      { 属性: 属性类型.内功基础攻击, 值: 952 },
      { 属性: 属性类型.内功破防等级, 值: 2453 },
      { 属性: 属性类型.破招值, 值: 2180 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 92492,
    uid: 202688,
    装备名称: '沉龙荡波·沧远腰带',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2521 },
      { 属性: 属性类型.元气, 值: 489 },
      { 属性: 属性类型.内功基础攻击, 值: 952 },
      { 属性: 属性类型.内功破防等级, 值: 2453 },
      { 属性: 属性类型.破招值, 值: 2180 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 94096,
    uid: 207375,
    装备名称: '翡翠御厨腰带·火候',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 10000,
    装备特效: 装备特效枚举.冬至套装,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2521 },
      { 属性: 属性类型.元气, 值: 489 },
      { 属性: 属性类型.内功基础攻击, 值: 952 },
      { 属性: 属性类型.内功会心等级, 值: 2453 },
      { 属性: 属性类型.破招值, 值: 2180 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 94119,
    uid: 207398,
    装备名称: '寻龙御厨腰带·火候',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 10000,
    装备特效: 装备特效枚举.冬至套装,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2521 },
      { 属性: 属性类型.元气, 值: 489 },
      { 属性: 属性类型.内功基础攻击, 值: 952 },
      { 属性: 属性类型.内功会心等级, 值: 2453 },
      { 属性: 属性类型.破招值, 值: 2180 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
  {
    id: 94120,
    uid: 207399,
    装备名称: '迎新御厨腰带·火候',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 10000,
    装备特效: 装备特效枚举.冬至套装,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 2521 },
      { 属性: 属性类型.根骨, 值: 489 },
      { 属性: 属性类型.内功基础攻击, 值: 952 },
      { 属性: 属性类型.内功会心等级, 值: 2453 },
      { 属性: 属性类型.破招值, 值: 2180 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心效果等级 }, { 镶嵌类型: 属性类型.内功会心等级 }],
  },
]

export default 腰带装备数据
