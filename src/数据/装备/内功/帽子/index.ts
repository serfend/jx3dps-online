import { 属性类型 } from '@/@types/属性'

import { 装备属性信息模型, 装备特效枚举, 装备类型枚举 } from '@/@types/装备'

const 帽子装备数据: 装备属性信息模型[] = [
  {
    id: 98502,
    uid: 215190,
    装备名称: '水川冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 15800,
    装备特效: 装备特效枚举.切糕_英雄,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5121 },
      { 属性: 属性类型.根骨, 值: 993 },
      { 属性: 属性类型.内功基础攻击, 值: 1933 },
      { 属性: 属性类型.内功会心等级, 值: 4982 },
      { 属性: 属性类型.破招值, 值: 4429 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 98503,
    uid: 215191,
    装备名称: '水泓冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 15800,
    装备特效: 装备特效枚举.切糕_英雄,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5121 },
      { 属性: 属性类型.元气, 值: 993 },
      { 属性: 属性类型.内功基础攻击, 值: 1933 },
      { 属性: 属性类型.全会心等级, 值: 4982 },
      { 属性: 属性类型.破招值, 值: 4429 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 100116,
    uid: 217044,
    装备名称: '陶然意·和时帽',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 15700,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5088 },
      { 属性: 属性类型.根骨, 值: 925 },
      { 属性: 属性类型.内功基础攻击, 值: 1477 },
      { 属性: 属性类型.内功会心等级, 值: 4951 },
      { 属性: 属性类型.全能等级, 值: 2200 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功会心效果等级 }],
  },
  {
    id: 100115,
    uid: 217043,
    装备名称: '陶然意·星雪帽',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 15700,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5088 },
      { 属性: 属性类型.元气, 值: 925 },
      { 属性: 属性类型.内功基础攻击, 值: 1477 },
      { 属性: 属性类型.内功会心等级, 值: 4951 },
      { 属性: 属性类型.全能等级, 值: 2200 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功会心效果等级 }],
  },
  {
    id: 100099,
    uid: 217027,
    装备名称: '陶然意·兰谷帽',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 15700,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5088 },
      { 属性: 属性类型.元气, 值: 925 },
      { 属性: 属性类型.内功基础攻击, 值: 1477 },
      { 属性: 属性类型.内功会心等级, 值: 4951 },
      { 属性: 属性类型.全能等级, 值: 2200 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功会心效果等级 }],
  },
  {
    id: 98394,
    uid: 214898,
    装备名称: '绕城冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5056 },
      { 属性: 属性类型.根骨, 值: 980 },
      { 属性: 属性类型.内功基础攻击, 值: 1908 },
      { 属性: 属性类型.内功会心等级, 值: 4919 },
      { 属性: 属性类型.无双等级, 值: 4373 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 98395,
    uid: 214899,
    装备名称: '宫阙冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5056 },
      { 属性: 属性类型.元气, 值: 980 },
      { 属性: 属性类型.内功基础攻击, 值: 1908 },
      { 属性: 属性类型.全会心等级, 值: 4919 },
      { 属性: 属性类型.无双等级, 值: 4373 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 98430,
    uid: 214958,
    装备名称: '情义冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5056 },
      { 属性: 属性类型.根骨, 值: 980 },
      { 属性: 属性类型.内功基础攻击, 值: 1908 },
      { 属性: 属性类型.加速等级, 值: 4919 },
      { 属性: 属性类型.破招值, 值: 4373 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 98431,
    uid: 214959,
    装备名称: '良安冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5056 },
      { 属性: 属性类型.元气, 值: 980 },
      { 属性: 属性类型.内功基础攻击, 值: 1908 },
      { 属性: 属性类型.加速等级, 值: 4919 },
      { 属性: 属性类型.破招值, 值: 4373 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 98526,
    uid: 215245,
    装备名称: '客行江湖·风翎冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5056 },
      { 属性: 属性类型.根骨, 值: 980 },
      { 属性: 属性类型.内功基础攻击, 值: 1908 },
      { 属性: 属性类型.内功会心等级, 值: 4919 },
      { 属性: 属性类型.破招值, 值: 4373 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 98527,
    uid: 215246,
    装备名称: '客行江湖·磐气冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5056 },
      { 属性: 属性类型.元气, 值: 980 },
      { 属性: 属性类型.内功基础攻击, 值: 1908 },
      { 属性: 属性类型.全会心等级, 值: 4919 },
      { 属性: 属性类型.破招值, 值: 4373 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 98604,
    uid: 215365,
    装备名称: '月纤冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5056 },
      { 属性: 属性类型.根骨, 值: 980 },
      { 属性: 属性类型.内功基础攻击, 值: 1908 },
      { 属性: 属性类型.内功会心等级, 值: 4919 },
      { 属性: 属性类型.破招值, 值: 4373 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 98605,
    uid: 215366,
    装备名称: '月迟冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 15600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 5056 },
      { 属性: 属性类型.元气, 值: 980 },
      { 属性: 属性类型.内功基础攻击, 值: 1908 },
      { 属性: 属性类型.全会心等级, 值: 4919 },
      { 属性: 属性类型.破招值, 值: 4373 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 98312,
    uid: 214816,
    装备名称: '鸿辉·鹿喧冠',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 15400,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4991 },
      { 属性: 属性类型.元气, 值: 968 },
      { 属性: 属性类型.内功基础攻击, 值: 1884 },
      { 属性: 属性类型.内功破防等级, 值: 4856 },
      { 属性: 属性类型.破招值, 值: 4316 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98335,
    uid: 214839,
    装备名称: '鸿辉·梦辰冠',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 15400,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4991 },
      { 属性: 属性类型.元气, 值: 968 },
      { 属性: 属性类型.内功基础攻击, 值: 1884 },
      { 属性: 属性类型.内功破防等级, 值: 4856 },
      { 属性: 属性类型.破招值, 值: 4316 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98336,
    uid: 214840,
    装备名称: '鸿辉·白林冠',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 15400,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4991 },
      { 属性: 属性类型.根骨, 值: 968 },
      { 属性: 属性类型.内功基础攻击, 值: 1884 },
      { 属性: 属性类型.内功破防等级, 值: 4856 },
      { 属性: 属性类型.破招值, 值: 4316 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98556,
    uid: 215299,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 15200,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 4863 },
      { 属性: 属性类型.无双等级, 值: 9453 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 98557,
    uid: 215300,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 15200,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 4148 },
      { 属性: 属性类型.全会心等级, 值: 5326 },
      { 属性: 属性类型.无双等级, 值: 5592 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 98558,
    uid: 215301,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 15200,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 4577 },
      { 属性: 属性类型.破招值, 值: 3195 },
      { 属性: 属性类型.内功破防等级, 值: 3462 },
      { 属性: 属性类型.全会心等级, 值: 3462 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 98532,
    uid: 215257,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 14350,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 4591 },
      { 属性: 属性类型.内功破防等级, 值: 8924 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.破招值 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98533,
    uid: 215258,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 14350,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3916 },
      { 属性: 属性类型.全会心等级, 值: 6285 },
      { 属性: 属性类型.全会心效果等级, 值: 3771 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98534,
    uid: 215259,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 14350,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3916 },
      { 属性: 属性类型.破招值, 值: 2765 },
      { 属性: 属性类型.全会心等级, 值: 4776 },
      { 属性: 属性类型.无双等级, 值: 2765 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96388,
    uid: 212113,
    装备名称: '风轻冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 14150,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4586 },
      { 属性: 属性类型.根骨, 值: 889 },
      { 属性: 属性类型.内功基础攻击, 值: 1731 },
      { 属性: 属性类型.内功会心等级, 值: 4462 },
      { 属性: 属性类型.破招值, 值: 3966 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 96389,
    uid: 212114,
    装备名称: '风绫冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 14150,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4586 },
      { 属性: 属性类型.元气, 值: 889 },
      { 属性: 属性类型.内功基础攻击, 值: 1731 },
      { 属性: 属性类型.全会心等级, 值: 4462 },
      { 属性: 属性类型.破招值, 值: 3966 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 99973,
    uid: 217027,
    装备名称: '陶然意·兰谷帽',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 14050,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4553 },
      { 属性: 属性类型.元气, 值: 828 },
      { 属性: 属性类型.内功基础攻击, 值: 1322 },
      { 属性: 属性类型.内功会心等级, 值: 4430 },
      { 属性: 属性类型.全能等级, 值: 1969 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功会心效果等级 }],
  },
  {
    id: 99989,
    uid: 217043,
    装备名称: '陶然意·星雪帽',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 14050,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4553 },
      { 属性: 属性类型.元气, 值: 828 },
      { 属性: 属性类型.内功基础攻击, 值: 1322 },
      { 属性: 属性类型.内功会心等级, 值: 4430 },
      { 属性: 属性类型.全能等级, 值: 1969 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功会心效果等级 }],
  },
  {
    id: 99990,
    uid: 217044,
    装备名称: '陶然意·和时帽',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 14050,
    装备类型: 装备类型枚举.PVX,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4553 },
      { 属性: 属性类型.根骨, 值: 828 },
      { 属性: 属性类型.内功基础攻击, 值: 1322 },
      { 属性: 属性类型.内功会心等级, 值: 4430 },
      { 属性: 属性类型.全能等级, 值: 1969 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功会心效果等级 }],
  },
  {
    id: 96280,
    uid: 211821,
    装备名称: '扬英冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.根骨, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.内功会心等级, 值: 4399 },
      { 属性: 属性类型.无双等级, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96281,
    uid: 211822,
    装备名称: '开颐冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.元气, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.全会心等级, 值: 4399 },
      { 属性: 属性类型.无双等级, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96316,
    uid: 211881,
    装备名称: '寒绡冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.根骨, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.加速等级, 值: 4399 },
      { 属性: 属性类型.破招值, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 96317,
    uid: 211882,
    装备名称: '壑云冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.元气, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.加速等级, 值: 4399 },
      { 属性: 属性类型.破招值, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 96412,
    uid: 212168,
    装备名称: '客行江湖·意悠冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.根骨, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.内功会心等级, 值: 4399 },
      { 属性: 属性类型.破招值, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 96413,
    uid: 212169,
    装备名称: '客行江湖·断意冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.元气, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.全会心等级, 值: 4399 },
      { 属性: 属性类型.破招值, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 96498,
    uid: 212302,
    装备名称: '泉合冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.根骨, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.内功会心等级, 值: 4399 },
      { 属性: 属性类型.破招值, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 96499,
    uid: 212303,
    装备名称: '泉麓冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.元气, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.全会心等级, 值: 4399 },
      { 属性: 属性类型.破招值, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 98208,
    uid: 214663,
    装备名称: '危音帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.根骨, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.内功破防等级, 值: 4399 },
      { 属性: 属性类型.破招值, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98209,
    uid: 214664,
    装备名称: '危影帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.元气, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.内功破防等级, 值: 4399 },
      { 属性: 属性类型.破招值, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98700,
    uid: 215562,
    装备名称: '东方日出·所适冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.根骨, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.内功会心等级, 值: 4399 },
      { 属性: 属性类型.无双等级, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 98701,
    uid: 215563,
    装备名称: '东方日出·当楼冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13950,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4521 },
      { 属性: 属性类型.元气, 值: 877 },
      { 属性: 属性类型.内功基础攻击, 值: 1707 },
      { 属性: 属性类型.全会心等级, 值: 4399 },
      { 属性: 属性类型.无双等级, 值: 3910 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96198,
    uid: 211739,
    装备名称: '灵源·月胧冠',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 13750,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4456 },
      { 属性: 属性类型.元气, 值: 864 },
      { 属性: 属性类型.内功基础攻击, 值: 1682 },
      { 属性: 属性类型.内功破防等级, 值: 4336 },
      { 属性: 属性类型.破招值, 值: 3854 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96221,
    uid: 211762,
    装备名称: '灵源·沉辉冠',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 13750,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4456 },
      { 属性: 属性类型.元气, 值: 864 },
      { 属性: 属性类型.内功基础攻击, 值: 1682 },
      { 属性: 属性类型.内功破防等级, 值: 4336 },
      { 属性: 属性类型.破招值, 值: 3854 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96222,
    uid: 211763,
    装备名称: '灵源·采芳冠',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 13750,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4456 },
      { 属性: 属性类型.根骨, 值: 864 },
      { 属性: 属性类型.内功基础攻击, 值: 1682 },
      { 属性: 属性类型.内功破防等级, 值: 4336 },
      { 属性: 属性类型.破招值, 值: 3854 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 98178,
    uid: 214602,
    装备名称: '寻踪觅宝·折月帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 13750,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4456 },
      { 属性: 属性类型.根骨, 值: 864 },
      { 属性: 属性类型.内功基础攻击, 值: 1682 },
      { 属性: 属性类型.内功会心等级, 值: 4336 },
      { 属性: 属性类型.无双等级, 值: 3854 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 98179,
    uid: 214603,
    装备名称: '寻踪觅宝·星辉帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 13750,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4456 },
      { 属性: 属性类型.元气, 值: 864 },
      { 属性: 属性类型.内功基础攻击, 值: 1682 },
      { 属性: 属性类型.全会心等级, 值: 4336 },
      { 属性: 属性类型.无双等级, 值: 3854 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96450,
    uid: 212236,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 13550,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 4335 },
      { 属性: 属性类型.全会心等级, 值: 8427 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 96451,
    uid: 212237,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 13550,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3698 },
      { 属性: 属性类型.破招值, 值: 4985 },
      { 属性: 属性类型.全会心等级, 值: 4747 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.破招值 }],
  },
  {
    id: 96452,
    uid: 212238,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 13550,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3698 },
      { 属性: 属性类型.内功破防等级, 值: 3323 },
      { 属性: 属性类型.全会心等级, 值: 3798 },
      { 属性: 属性类型.无双等级, 值: 2611 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96418,
    uid: 212180,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 12800,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 4096 },
      { 属性: 属性类型.无双等级, 值: 7960 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96419,
    uid: 212181,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 12800,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3493 },
      { 属性: 属性类型.全会心等级, 值: 4485 },
      { 属性: 属性类型.无双等级, 值: 4709 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96420,
    uid: 212182,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 12800,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3855 },
      { 属性: 属性类型.破招值, 值: 2691 },
      { 属性: 属性类型.内功破防等级, 值: 2915 },
      { 属性: 属性类型.全会心等级, 值: 2915 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 94485,
    uid: 208179,
    装备名称: '雪满冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12600,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4083 },
      { 属性: 属性类型.根骨, 值: 792 },
      { 属性: 属性类型.内功基础攻击, 值: 1541 },
      { 属性: 属性类型.内功会心等级, 值: 3973 },
      { 属性: 属性类型.破招值, 值: 3532 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 94486,
    uid: 208180,
    装备名称: '雪洁冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12600,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4083 },
      { 属性: 属性类型.元气, 值: 792 },
      { 属性: 属性类型.内功基础攻击, 值: 1541 },
      { 属性: 属性类型.全会心等级, 值: 3973 },
      { 属性: 属性类型.破招值, 值: 3532 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 94389,
    uid: 207905,
    装备名称: '池泓冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4035 },
      { 属性: 属性类型.根骨, 值: 782 },
      { 属性: 属性类型.内功基础攻击, 值: 1523 },
      { 属性: 属性类型.内功会心等级, 值: 3926 },
      { 属性: 属性类型.无双等级, 值: 3490 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 94390,
    uid: 207906,
    装备名称: '椴微冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4035 },
      { 属性: 属性类型.元气, 值: 782 },
      { 属性: 属性类型.内功基础攻击, 值: 1523 },
      { 属性: 属性类型.全会心等级, 值: 3926 },
      { 属性: 属性类型.无双等级, 值: 3490 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 94425,
    uid: 207965,
    装备名称: '朝华冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4035 },
      { 属性: 属性类型.根骨, 值: 782 },
      { 属性: 属性类型.内功基础攻击, 值: 1523 },
      { 属性: 属性类型.加速等级, 值: 3926 },
      { 属性: 属性类型.破招值, 值: 3490 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 94426,
    uid: 207966,
    装备名称: '沁渡冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4035 },
      { 属性: 属性类型.元气, 值: 782 },
      { 属性: 属性类型.内功基础攻击, 值: 1523 },
      { 属性: 属性类型.加速等级, 值: 3926 },
      { 属性: 属性类型.破招值, 值: 3490 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 94509,
    uid: 208233,
    装备名称: '客行江湖·渐浓冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4035 },
      { 属性: 属性类型.根骨, 值: 782 },
      { 属性: 属性类型.内功基础攻击, 值: 1523 },
      { 属性: 属性类型.内功会心等级, 值: 3926 },
      { 属性: 属性类型.破招值, 值: 3490 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 94510,
    uid: 208234,
    装备名称: '客行江湖·千世冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4035 },
      { 属性: 属性类型.元气, 值: 782 },
      { 属性: 属性类型.内功基础攻击, 值: 1523 },
      { 属性: 属性类型.全会心等级, 值: 3926 },
      { 属性: 属性类型.破招值, 值: 3490 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 94587,
    uid: 208353,
    装备名称: '湖寂冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4035 },
      { 属性: 属性类型.根骨, 值: 782 },
      { 属性: 属性类型.内功基础攻击, 值: 1523 },
      { 属性: 属性类型.内功会心等级, 值: 3926 },
      { 属性: 属性类型.破招值, 值: 3490 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 94588,
    uid: 208354,
    装备名称: '湖烟冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4035 },
      { 属性: 属性类型.元气, 值: 782 },
      { 属性: 属性类型.内功基础攻击, 值: 1523 },
      { 属性: 属性类型.全会心等级, 值: 3926 },
      { 属性: 属性类型.破招值, 值: 3490 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 96594,
    uid: 212499,
    装备名称: '西风北啸·音书冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4035 },
      { 属性: 属性类型.根骨, 值: 782 },
      { 属性: 属性类型.内功基础攻击, 值: 1523 },
      { 属性: 属性类型.内功会心等级, 值: 3926 },
      { 属性: 属性类型.无双等级, 值: 3490 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96595,
    uid: 212500,
    装备名称: '西风北啸·离声冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12450,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4035 },
      { 属性: 属性类型.元气, 值: 782 },
      { 属性: 属性类型.内功基础攻击, 值: 1523 },
      { 属性: 属性类型.全会心等级, 值: 3926 },
      { 属性: 属性类型.无双等级, 值: 3490 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 90525,
    uid: 189017,
    装备名称: '临可帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12400,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4019 },
      { 属性: 属性类型.根骨, 值: 779 },
      { 属性: 属性类型.内功基础攻击, 值: 1517 },
      { 属性: 属性类型.内功破防等级, 值: 3910 },
      { 属性: 属性类型.无双等级, 值: 3476 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90526,
    uid: 189018,
    装备名称: '临起帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12400,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 4019 },
      { 属性: 属性类型.元气, 值: 779 },
      { 属性: 属性类型.内功基础攻击, 值: 1517 },
      { 属性: 属性类型.内功破防等级, 值: 3910 },
      { 属性: 属性类型.无双等级, 值: 3476 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90351,
    uid: 188609,
    装备名称: '德襄帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.根骨, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.内功会心等级, 值: 3879 },
      { 属性: 属性类型.破招值, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90352,
    uid: 188610,
    装备名称: '烟琐帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.全会心等级, 值: 3879 },
      { 属性: 属性类型.破招值, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90387,
    uid: 188699,
    装备名称: '擒雨帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.根骨, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.加速等级, 值: 3879 },
      { 属性: 属性类型.无双等级, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90388,
    uid: 188700,
    装备名称: '语阔帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.加速等级, 值: 3879 },
      { 属性: 属性类型.无双等级, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.无双等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90423,
    uid: 188789,
    装备名称: '忆安冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.根骨, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.内功破防等级, 值: 3879 },
      { 属性: 属性类型.破招值, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90424,
    uid: 188790,
    装备名称: '忆惜冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.内功破防等级, 值: 3879 },
      { 属性: 属性类型.破招值, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90555,
    uid: 189071,
    装备名称: '故云冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.根骨, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.内功会心等级, 值: 3879 },
      { 属性: 属性类型.无双等级, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 90556,
    uid: 189072,
    装备名称: '庭澜冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.全会心等级, 值: 3879 },
      { 属性: 属性类型.无双等级, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 90591,
    uid: 189161,
    装备名称: '途南冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.根骨, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.加速等级, 值: 3879 },
      { 属性: 属性类型.破招值, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 90592,
    uid: 189162,
    装备名称: '花霭冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.加速等级, 值: 3879 },
      { 属性: 属性类型.破招值, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 90627,
    uid: 189251,
    装备名称: '剪桐冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.根骨, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.内功会心等级, 值: 3879 },
      { 属性: 属性类型.破招值, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90628,
    uid: 189252,
    装备名称: '故芳冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.全会心等级, 值: 3879 },
      { 属性: 属性类型.破招值, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90663,
    uid: 189341,
    装备名称: '谨峰冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.根骨, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.内功破防等级, 值: 3879 },
      { 属性: 属性类型.无双等级, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90664,
    uid: 189342,
    装备名称: '藏峦冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.内功破防等级, 值: 3879 },
      { 属性: 属性类型.无双等级, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 94309,
    uid: 207825,
    装备名称: '濯心·松声冠',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 12300,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.内功破防等级, 值: 3879 },
      { 属性: 属性类型.破招值, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94332,
    uid: 207848,
    装备名称: '濯心·天尘冠',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 12300,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.内功破防等级, 值: 3879 },
      { 属性: 属性类型.破招值, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94333,
    uid: 207849,
    装备名称: '濯心·采青冠',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 12300,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.根骨, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.内功破防等级, 值: 3879 },
      { 属性: 属性类型.破招值, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96094,
    uid: 211604,
    装备名称: '寻踪觅宝·拂雪帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.根骨, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.内功会心等级, 值: 3879 },
      { 属性: 属性类型.无双等级, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 96095,
    uid: 211605,
    装备名称: '寻踪觅宝·泻雨帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 773 },
      { 属性: 属性类型.内功基础攻击, 值: 1505 },
      { 属性: 属性类型.全会心等级, 值: 3879 },
      { 属性: 属性类型.无双等级, 值: 3448 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 100357,
    uid: 217832,
    装备名称: '若寓帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 725 },
      { 属性: 属性类型.内功基础攻击, 值: 1158 },
      { 属性: 属性类型.加速等级, 值: 3879 },
      { 属性: 属性类型.全能等级, 值: 1724 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.额外气血上限 }],
  },
  {
    id: 100356,
    uid: 217831,
    装备名称: '越风帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.根骨, 值: 725 },
      { 属性: 属性类型.内功基础攻击, 值: 1158 },
      { 属性: 属性类型.加速等级, 值: 3879 },
      { 属性: 属性类型.全能等级, 值: 1724 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.额外气血上限 }],
  },
  {
    id: 100321,
    uid: 217741,
    装备名称: '踏江帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.元气, 值: 725 },
      { 属性: 属性类型.内功基础攻击, 值: 1158 },
      { 属性: 属性类型.全会心等级, 值: 3879 },
      { 属性: 属性类型.全能等级, 值: 1724 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.全会心效果等级 }],
  },
  {
    id: 100320,
    uid: 217740,
    装备名称: '承康帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 12300,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3986 },
      { 属性: 属性类型.根骨, 值: 725 },
      { 属性: 属性类型.内功基础攻击, 值: 1158 },
      { 属性: 属性类型.内功会心等级, 值: 3879 },
      { 属性: 属性类型.全能等级, 值: 1724 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功会心效果等级 }],
  },
  {
    id: 94539,
    uid: 208287,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 12100,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3872 },
      { 属性: 属性类型.内功破防等级, 值: 7525 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.破招值 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94540,
    uid: 208288,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 12100,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3302 },
      { 属性: 属性类型.全会心等级, 值: 5299 },
      { 属性: 属性类型.全会心效果等级, 值: 3180 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94541,
    uid: 208289,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 12100,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3302 },
      { 属性: 属性类型.破招值, 值: 2332 },
      { 属性: 属性类型.全会心等级, 值: 4027 },
      { 属性: 属性类型.无双等级, 值: 2332 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 90501,
    uid: 188969,
    装备名称: '临可帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11800,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3824 },
      { 属性: 属性类型.根骨, 值: 742 },
      { 属性: 属性类型.内功基础攻击, 值: 1444 },
      { 属性: 属性类型.内功破防等级, 值: 3721 },
      { 属性: 属性类型.无双等级, 值: 3307 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90502,
    uid: 188970,
    装备名称: '临起帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11800,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3824 },
      { 属性: 属性类型.元气, 值: 742 },
      { 属性: 属性类型.内功基础攻击, 值: 1444 },
      { 属性: 属性类型.内功破防等级, 值: 3721 },
      { 属性: 属性类型.无双等级, 值: 3307 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 96064,
    uid: 211525,
    装备名称: '虚璜帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11800,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3824 },
      { 属性: 属性类型.根骨, 值: 742 },
      { 属性: 属性类型.内功基础攻击, 值: 1444 },
      { 属性: 属性类型.内功破防等级, 值: 3721 },
      { 属性: 属性类型.破招值, 值: 3307 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 96065,
    uid: 211526,
    装备名称: '虚珩帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11800,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3824 },
      { 属性: 属性类型.元气, 值: 742 },
      { 属性: 属性类型.内功基础攻击, 值: 1444 },
      { 属性: 属性类型.内功破防等级, 值: 3721 },
      { 属性: 属性类型.破招值, 值: 3307 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 93505,
    uid: 204779,
    装备名称: '月阑冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3727 },
      { 属性: 属性类型.根骨, 值: 723 },
      { 属性: 属性类型.内功基础攻击, 值: 1407 },
      { 属性: 属性类型.内功会心等级, 值: 3626 },
      { 属性: 属性类型.破招值, 值: 3223 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 93506,
    uid: 204780,
    装备名称: '轻船冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3727 },
      { 属性: 属性类型.元气, 值: 723 },
      { 属性: 属性类型.内功基础攻击, 值: 1407 },
      { 属性: 属性类型.全会心等级, 值: 3626 },
      { 属性: 属性类型.破招值, 值: 3223 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 94515,
    uid: 208245,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 11500,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3680 },
      { 属性: 属性类型.全会心等级, 值: 7152 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 94516,
    uid: 208246,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 11500,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3138 },
      { 属性: 属性类型.破招值, 值: 4231 },
      { 属性: 属性类型.全会心等级, 值: 4029 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.破招值 }],
  },
  {
    id: 94517,
    uid: 208247,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 11500,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3138 },
      { 属性: 属性类型.内功破防等级, 值: 2820 },
      { 属性: 属性类型.全会心等级, 值: 3223 },
      { 属性: 属性类型.无双等级, 值: 2216 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91313,
    uid: 200483,
    装备名称: '暮漠冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11300,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3662 },
      { 属性: 属性类型.根骨, 值: 710 },
      { 属性: 属性类型.内功基础攻击, 值: 1382 },
      { 属性: 属性类型.内功会心等级, 值: 3563 },
      { 属性: 属性类型.破招值, 值: 3167 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 91314,
    uid: 200484,
    装备名称: '暮盈冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11300,
    装备特效: 装备特效枚举.切糕_普通,
    装备类型: 装备类型枚举.切糕,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3662 },
      { 属性: 属性类型.元气, 值: 710 },
      { 属性: 属性类型.内功基础攻击, 值: 1382 },
      { 属性: 属性类型.全会心等级, 值: 3563 },
      { 属性: 属性类型.破招值, 值: 3167 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90477,
    uid: 188921,
    装备名称: '临可帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11200,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3630 },
      { 属性: 属性类型.根骨, 值: 704 },
      { 属性: 属性类型.内功基础攻击, 值: 1370 },
      { 属性: 属性类型.内功破防等级, 值: 3532 },
      { 属性: 属性类型.无双等级, 值: 3139 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90478,
    uid: 188922,
    装备名称: '临起帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11200,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3630 },
      { 属性: 属性类型.元气, 值: 704 },
      { 属性: 属性类型.内功基础攻击, 值: 1370 },
      { 属性: 属性类型.内功破防等级, 值: 3532 },
      { 属性: 属性类型.无双等级, 值: 3139 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90976,
    uid: 189876,
    装备名称: '湘沅冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3614 },
      { 属性: 属性类型.根骨, 值: 701 },
      { 属性: 属性类型.内功基础攻击, 值: 1364 },
      { 属性: 属性类型.内功会心等级, 值: 3516 },
      { 属性: 属性类型.破招值, 值: 3125 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90977,
    uid: 189877,
    装备名称: '湘崖冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3614 },
      { 属性: 属性类型.元气, 值: 701 },
      { 属性: 属性类型.内功基础攻击, 值: 1364 },
      { 属性: 属性类型.全会心等级, 值: 3516 },
      { 属性: 属性类型.破招值, 值: 3125 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 91217,
    uid: 200209,
    装备名称: '晓菡冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3614 },
      { 属性: 属性类型.根骨, 值: 701 },
      { 属性: 属性类型.内功基础攻击, 值: 1364 },
      { 属性: 属性类型.内功会心等级, 值: 3516 },
      { 属性: 属性类型.无双等级, 值: 3125 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 91218,
    uid: 200210,
    装备名称: '凝青冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3614 },
      { 属性: 属性类型.元气, 值: 701 },
      { 属性: 属性类型.内功基础攻击, 值: 1364 },
      { 属性: 属性类型.全会心等级, 值: 3516 },
      { 属性: 属性类型.无双等级, 值: 3125 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 91253,
    uid: 200269,
    装备名称: '思菱帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3614 },
      { 属性: 属性类型.根骨, 值: 701 },
      { 属性: 属性类型.内功基础攻击, 值: 1364 },
      { 属性: 属性类型.加速等级, 值: 3516 },
      { 属性: 属性类型.破招值, 值: 3125 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 91254,
    uid: 200270,
    装备名称: '白萱帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3614 },
      { 属性: 属性类型.元气, 值: 701 },
      { 属性: 属性类型.内功基础攻击, 值: 1364 },
      { 属性: 属性类型.加速等级, 值: 3516 },
      { 属性: 属性类型.破招值, 值: 3125 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 91355,
    uid: 200561,
    装备名称: '客行江湖·惊憩冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3614 },
      { 属性: 属性类型.根骨, 值: 701 },
      { 属性: 属性类型.内功基础攻击, 值: 1364 },
      { 属性: 属性类型.内功会心等级, 值: 3516 },
      { 属性: 属性类型.破招值, 值: 3125 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 91356,
    uid: 200562,
    装备名称: '客行江湖·凝章冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3614 },
      { 属性: 属性类型.元气, 值: 701 },
      { 属性: 属性类型.内功基础攻击, 值: 1364 },
      { 属性: 属性类型.全会心等级, 值: 3516 },
      { 属性: 属性类型.破招值, 值: 3125 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90279,
    uid: 188429,
    装备名称: '岭深帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.根骨, 值: 691 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.加速等级, 值: 3469 },
      { 属性: 属性类型.破招值, 值: 3083 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 90280,
    uid: 188430,
    装备名称: '散幽帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.元气, 值: 691 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.加速等级, 值: 3469 },
      { 属性: 属性类型.破招值, 值: 3083 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 91137,
    uid: 200129,
    装备名称: '揽江·落华冠',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 11000,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.元气, 值: 691 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.内功破防等级, 值: 3469 },
      { 属性: 属性类型.破招值, 值: 3083 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91160,
    uid: 200152,
    装备名称: '揽江·玉玑冠',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 11000,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.元气, 值: 691 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.内功破防等级, 值: 3469 },
      { 属性: 属性类型.破招值, 值: 3083 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91161,
    uid: 200153,
    装备名称: '揽江·柳颦冠',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 11000,
    装备特效: 装备特效枚举.门派套装_两件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.根骨, 值: 691 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.内功破防等级, 值: 3469 },
      { 属性: 属性类型.破招值, 值: 3083 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94208,
    uid: 207694,
    装备名称: '寻踪觅宝·花狸帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.根骨, 值: 691 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.内功会心等级, 值: 3469 },
      { 属性: 属性类型.无双等级, 值: 3083 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 94209,
    uid: 207695,
    装备名称: '寻踪觅宝·细雨帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.元气, 值: 691 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.全会心等级, 值: 3469 },
      { 属性: 属性类型.无双等级, 值: 3083 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 94683,
    uid: 208565,
    装备名称: '风销残烟·幽吹冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.根骨, 值: 691 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.内功会心等级, 值: 3469 },
      { 属性: 属性类型.无双等级, 值: 3083 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 94684,
    uid: 208566,
    装备名称: '风销残烟·樗闲冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.元气, 值: 691 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.全会心等级, 值: 3469 },
      { 属性: 属性类型.无双等级, 值: 3083 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 100284,
    uid: 217649,
    装备名称: '丹风帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.根骨, 值: 691 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.内功会心等级, 值: 3469 },
      { 属性: 属性类型.破招值, 值: 3083 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 100285,
    uid: 217650,
    装备名称: '神逸帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.元气, 值: 691 },
      { 属性: 属性类型.内功基础攻击, 值: 1346 },
      { 属性: 属性类型.全会心等级, 值: 3469 },
      { 属性: 属性类型.破招值, 值: 3083 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 100483,
    uid: 218134,
    装备名称: '神逸帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.元气, 值: 648 },
      { 属性: 属性类型.内功基础攻击, 值: 1035 },
      { 属性: 属性类型.全会心等级, 值: 3469 },
      { 属性: 属性类型.全能等级, 值: 1542 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.全会心效果等级 }],
  },
  {
    id: 100482,
    uid: 218133,
    装备名称: '丹风帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 11000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3565 },
      { 属性: 属性类型.根骨, 值: 648 },
      { 属性: 属性类型.内功基础攻击, 值: 1035 },
      { 属性: 属性类型.内功会心等级, 值: 3469 },
      { 属性: 属性类型.全能等级, 值: 1542 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功会心等级 }, { 镶嵌类型: 属性类型.内功会心效果等级 }],
  },
  {
    id: 91553,
    uid: 200909,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10900,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3488 },
      { 属性: 属性类型.无双等级, 值: 6588 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 91554,
    uid: 200910,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10900,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 2975 },
      { 属性: 属性类型.全会心等级, 值: 3819 },
      { 属性: 属性类型.无双等级, 值: 3819 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 91555,
    uid: 200911,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10900,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3282 },
      { 属性: 属性类型.破招值, 值: 2100 },
      { 属性: 属性类型.内功破防等级, 值: 2482 },
      { 属性: 属性类型.全会心等级, 值: 2482 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90243,
    uid: 188339,
    装备名称: '北池巾',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10800,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3500 },
      { 属性: 属性类型.根骨, 值: 679 },
      { 属性: 属性类型.内功基础攻击, 值: 1321 },
      { 属性: 属性类型.内功破防等级, 值: 3406 },
      { 属性: 属性类型.破招值, 值: 3027 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90244,
    uid: 188340,
    装备名称: '澹风巾',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10800,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3500 },
      { 属性: 属性类型.元气, 值: 679 },
      { 属性: 属性类型.内功基础攻击, 值: 1321 },
      { 属性: 属性类型.内功破防等级, 值: 3406 },
      { 属性: 属性类型.破招值, 值: 3027 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90453,
    uid: 188873,
    装备名称: '临可帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3435 },
      { 属性: 属性类型.根骨, 值: 666 },
      { 属性: 属性类型.内功基础攻击, 值: 1297 },
      { 属性: 属性类型.内功破防等级, 值: 3342 },
      { 属性: 属性类型.无双等级, 值: 2971 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90454,
    uid: 188874,
    装备名称: '临起帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10600,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3435 },
      { 属性: 属性类型.元气, 值: 666 },
      { 属性: 属性类型.内功基础攻击, 值: 1297 },
      { 属性: 属性类型.内功破防等级, 值: 3342 },
      { 属性: 属性类型.无双等级, 值: 2971 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 91521,
    uid: 200853,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10600,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3392 },
      { 属性: 属性类型.内功破防等级, 值: 6406 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.破招值 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91522,
    uid: 200854,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10600,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 2893 },
      { 属性: 属性类型.全会心等级, 值: 4457 },
      { 属性: 属性类型.全会心效果等级, 值: 2785 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91523,
    uid: 200855,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10600,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 2893 },
      { 属性: 属性类型.破招值, 值: 1857 },
      { 属性: 属性类型.全会心等级, 值: 3528 },
      { 属性: 属性类型.无双等级, 值: 2043 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 36600,
    uid: 119931,
    装备名称: '半夏帽',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3403 },
      { 属性: 属性类型.元气, 值: 619 },
      { 属性: 属性类型.内功基础攻击, 值: 988 },
      { 属性: 属性类型.内功破防等级, 值: 3311 },
      { 属性: 属性类型.全能等级, 值: 1472 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 52876,
    uid: 171357,
    装备名称: '勉学冠',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3403 },
      { 属性: 属性类型.元气, 值: 619 },
      { 属性: 属性类型.内功基础攻击, 值: 988 },
      { 属性: 属性类型.内功破防等级, 值: 3311 },
      { 属性: 属性类型.全能等级, 值: 1472 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 57198,
    uid: 181713,
    装备名称: '归林冠',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3403 },
      { 属性: 属性类型.根骨, 值: 619 },
      { 属性: 属性类型.内功基础攻击, 值: 988 },
      { 属性: 属性类型.内功破防等级, 值: 3311 },
      { 属性: 属性类型.全能等级, 值: 1472 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 90171,
    uid: 188159,
    装备名称: '凭心帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3403 },
      { 属性: 属性类型.根骨, 值: 619 },
      { 属性: 属性类型.内功基础攻击, 值: 988 },
      { 属性: 属性类型.内功破防等级, 值: 3311 },
      { 属性: 属性类型.全能等级, 值: 1472 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 90172,
    uid: 188160,
    装备名称: '无竟帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10500,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3403 },
      { 属性: 属性类型.元气, 值: 619 },
      { 属性: 属性类型.内功基础攻击, 值: 988 },
      { 属性: 属性类型.内功破防等级, 值: 3311 },
      { 属性: 属性类型.全能等级, 值: 1472 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.全能等级 }],
  },
  {
    id: 94178,
    uid: 207616,
    装备名称: '虚香帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10400,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3370 },
      { 属性: 属性类型.根骨, 值: 654 },
      { 属性: 属性类型.内功基础攻击, 值: 1272 },
      { 属性: 属性类型.内功破防等级, 值: 3279 },
      { 属性: 属性类型.破招值, 值: 2915 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 94179,
    uid: 207617,
    装备名称: '虚缘帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10400,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3370 },
      { 属性: 属性类型.元气, 值: 654 },
      { 属性: 属性类型.内功基础攻击, 值: 1272 },
      { 属性: 属性类型.内功破防等级, 值: 3279 },
      { 属性: 属性类型.破招值, 值: 2915 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91489,
    uid: 200797,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10300,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3296 },
      { 属性: 属性类型.全会心等级, 值: 6225 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.全会心等级 }],
  },
  {
    id: 91490,
    uid: 200798,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10300,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 2811 },
      { 属性: 属性类型.破招值, 值: 3609 },
      { 属性: 属性类型.全会心等级, 值: 3609 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.破招值 }],
  },
  {
    id: 91491,
    uid: 200799,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10300,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 2811 },
      { 属性: 属性类型.内功破防等级, 值: 2526 },
      { 属性: 属性类型.全会心等级, 值: 2887 },
      { 属性: 属性类型.无双等级, 值: 1804 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90904,
    uid: 189720,
    装备名称: '持溯冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3289 },
      { 属性: 属性类型.根骨, 值: 638 },
      { 属性: 属性类型.内功基础攻击, 值: 1242 },
      { 属性: 属性类型.内功会心等级, 值: 3201 },
      { 属性: 属性类型.无双等级, 值: 2845 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 90905,
    uid: 189721,
    装备名称: '青霍冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3289 },
      { 属性: 属性类型.元气, 值: 638 },
      { 属性: 属性类型.内功基础攻击, 值: 1242 },
      { 属性: 属性类型.全会心等级, 值: 3201 },
      { 属性: 属性类型.无双等级, 值: 2845 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 90940,
    uid: 189780,
    装备名称: '末闻帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3289 },
      { 属性: 属性类型.根骨, 值: 638 },
      { 属性: 属性类型.内功基础攻击, 值: 1242 },
      { 属性: 属性类型.加速等级, 值: 3201 },
      { 属性: 属性类型.破招值, 值: 2845 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.根骨 }],
  },
  {
    id: 90941,
    uid: 189781,
    装备名称: '啸影帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3289 },
      { 属性: 属性类型.元气, 值: 638 },
      { 属性: 属性类型.内功基础攻击, 值: 1242 },
      { 属性: 属性类型.加速等级, 值: 3201 },
      { 属性: 属性类型.破招值, 值: 2845 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.元气 }],
  },
  {
    id: 91337,
    uid: 200537,
    装备名称: '客行江湖·襄云冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3289 },
      { 属性: 属性类型.根骨, 值: 638 },
      { 属性: 属性类型.内功基础攻击, 值: 1242 },
      { 属性: 属性类型.内功会心等级, 值: 3201 },
      { 属性: 属性类型.破招值, 值: 2845 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 91338,
    uid: 200538,
    装备名称: '客行江湖·长绣冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10150,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3289 },
      { 属性: 属性类型.元气, 值: 638 },
      { 属性: 属性类型.内功基础攻击, 值: 1242 },
      { 属性: 属性类型.全会心等级, 值: 3201 },
      { 属性: 属性类型.破招值, 值: 2845 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功基础攻击 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 90723,
    uid: 189509,
    装备名称: '寻踪觅宝·君溪帽',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3241 },
      { 属性: 属性类型.根骨, 值: 628 },
      { 属性: 属性类型.内功基础攻击, 值: 1223 },
      { 属性: 属性类型.内功会心等级, 值: 3153 },
      { 属性: 属性类型.无双等级, 值: 2803 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 90724,
    uid: 189510,
    装备名称: '寻踪觅宝·莫南帽',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3241 },
      { 属性: 属性类型.元气, 值: 628 },
      { 属性: 属性类型.内功基础攻击, 值: 1223 },
      { 属性: 属性类型.全会心等级, 值: 3153 },
      { 属性: 属性类型.无双等级, 值: 2803 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 90824,
    uid: 189640,
    装备名称: '展锋·看妆冠',
    所属门派: '万花',
    装备主属性: '元气',
    装备品级: 10000,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3241 },
      { 属性: 属性类型.元气, 值: 628 },
      { 属性: 属性类型.内功基础攻击, 值: 1223 },
      { 属性: 属性类型.内功破防等级, 值: 3153 },
      { 属性: 属性类型.破招值, 值: 2803 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90847,
    uid: 189663,
    装备名称: '展锋·晓河冠',
    所属门派: '衍天',
    装备主属性: '元气',
    装备品级: 10000,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3241 },
      { 属性: 属性类型.元气, 值: 628 },
      { 属性: 属性类型.内功基础攻击, 值: 1223 },
      { 属性: 属性类型.内功破防等级, 值: 3153 },
      { 属性: 属性类型.破招值, 值: 2803 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 90848,
    uid: 189664,
    装备名称: '展锋·待芳冠',
    所属门派: '药宗',
    装备主属性: '根骨',
    装备品级: 10000,
    装备特效: 装备特效枚举.门派套装_四件套双会,
    装备类型: 装备类型枚举.门派套装,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3241 },
      { 属性: 属性类型.根骨, 值: 628 },
      { 属性: 属性类型.内功基础攻击, 值: 1223 },
      { 属性: 属性类型.内功破防等级, 值: 3153 },
      { 属性: 属性类型.破招值, 值: 2803 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功基础攻击 }],
  },
  {
    id: 91457,
    uid: 200741,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10000,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3200 },
      { 属性: 属性类型.无双等级, 值: 6044 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 91458,
    uid: 200742,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10000,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 2729 },
      { 属性: 属性类型.全会心等级, 值: 3504 },
      { 属性: 属性类型.无双等级, 值: 3504 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 91459,
    uid: 200743,
    装备名称: '无封头饰',
    所属门派: '通用',
    装备主属性: '通用',
    装备品级: 10000,
    装备类型: 装备类型枚举.试炼精简,
    装备增益: [
      { 属性: 属性类型.内功基础攻击, 值: 3011 },
      { 属性: 属性类型.破招值, 值: 1927 },
      { 属性: 属性类型.内功破防等级, 值: 2277 },
      { 属性: 属性类型.全会心等级, 值: 2277 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.全会心效果等级 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 92509,
    uid: 202705,
    装备名称: '沉龙荡波·漠天冠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3241 },
      { 属性: 属性类型.根骨, 值: 628 },
      { 属性: 属性类型.内功基础攻击, 值: 1223 },
      { 属性: 属性类型.内功会心等级, 值: 3153 },
      { 属性: 属性类型.无双等级, 值: 2803 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 92510,
    uid: 202706,
    装备名称: '沉龙荡波·沧远冠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3241 },
      { 属性: 属性类型.元气, 值: 628 },
      { 属性: 属性类型.内功基础攻击, 值: 1223 },
      { 属性: 属性类型.全会心等级, 值: 3153 },
      { 属性: 属性类型.无双等级, 值: 2803 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.内功破防等级 }, { 镶嵌类型: 属性类型.无双等级 }],
  },
  {
    id: 94154,
    uid: 207537,
    装备名称: '寻常斗笠',
    所属门派: '通用',
    装备主属性: '根骨',
    装备品级: 10000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3241 },
      { 属性: 属性类型.根骨, 值: 628 },
      { 属性: 属性类型.内功基础攻击, 值: 1223 },
      { 属性: 属性类型.内功破防等级, 值: 3153 },
      { 属性: 属性类型.无双等级, 值: 2803 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.根骨 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
  {
    id: 94155,
    uid: 207538,
    装备名称: '寻常斗笠',
    所属门派: '通用',
    装备主属性: '元气',
    装备品级: 10000,
    装备类型: 装备类型枚举.普通,
    装备增益: [
      { 属性: 属性类型.体质, 值: 3241 },
      { 属性: 属性类型.元气, 值: 628 },
      { 属性: 属性类型.内功基础攻击, 值: 1223 },
      { 属性: 属性类型.内功破防等级, 值: 3153 },
      { 属性: 属性类型.无双等级, 值: 2803 },
    ],
    镶嵌孔数组: [{ 镶嵌类型: 属性类型.元气 }, { 镶嵌类型: 属性类型.内功破防等级 }],
  },
]

export default 帽子装备数据
